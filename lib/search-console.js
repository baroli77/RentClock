...
function encryptionKey() {
  ...
  // RentClock already has a server-only secret. Derive a separate stable key
  // from it so the first Google connection needs no additional secret setup.
  if (process.env.SUPABASE_SERVICE_ROLE_KEY) {
    return crypto
      .createHash("sha256")
      .update(`rentclock-search-console-v1:${process.env.SUPABASE_SERVICE_ROLE_KEY}`)
      .digest();
  }
  throw new Error("A server-side encryption key has not been configured.");
}

...
-- Written only by the cron job via service role; no user policies needed.
...
create policy "certs read own"
  ...
  using (bucket_id = 'certs' and (storage.foldername(name))[1] = auth.uid()::text);
create policy "certs insert own"
  on storage.objects for insert
  with check (bucket_id = 'certs' and (storage.foldername(name))[1] = auth.uid()::text);
